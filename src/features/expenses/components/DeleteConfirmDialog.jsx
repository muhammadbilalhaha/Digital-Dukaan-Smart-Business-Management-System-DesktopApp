// src/features/expenses/components/DeleteConfirmDialog.jsx
import React from 'react';
import { Trash2, Loader2 } from 'lucide-react';

const DeleteConfirmDialog = ({ isOpen, onClose, onConfirm, expenseTitle, isLoading }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-card-bg rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center border border-border-light">
                ...
                <h3 className="text-lg font-bold text-text-primary mb-2">Delete Expense?</h3>
                <p className="text-sm text-text-muted mb-6">
                    Are you sure you want to permanently delete <strong>"{expenseTitle}"</strong>? This action cannot be undone.
                </p>
                <div className="flex gap-3">
                    ...
                    >
                        {isLoading ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={14} />}
                        Delete
                    </button>
                </div>
            </div>
