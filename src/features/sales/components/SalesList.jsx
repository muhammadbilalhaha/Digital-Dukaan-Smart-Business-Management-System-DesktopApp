// src/features/sales/components/SalesList.jsx
...
const StatusBadge = ({ paid, remaining }) => {
    if (remaining <= 0) return (
        <span className="inline-flex items-center gap-1 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 text-[10px] font-bold px-2.5 py-1 rounded-full border border-emerald-200">
            <CheckCircle2 size={10} /> Paid
        </span>
    );
    if (paid > 0 && remaining > 0) return (
        <span className="inline-flex items-center gap-1 bg-amber-50 dark:bg-amber-900/20 text-amber-700 text-[10px] font-bold px-2.5 py-1 rounded-full border border-amber-200">
            <AlertTriangle size={10} /> Partial
        </span>
    );
    return (
        <span className="inline-flex items-center gap-1 bg-red-50 dark:bg-red-900/20 text-red-700 text-[10px] font-bold px-2.5 py-1 rounded-full border border-red-200">
            <XCircle size={10} /> Unpaid
        </span>
    );
};

const SalesList = ({ sales, isLoading, onSaleClick }) => {
    const [currentPage, setCurrentPage] = useState(1);
    const totalPages = Math.ceil(sales.length / ITEMS_PER_PAGE);
    const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
    const paginated = sales.slice(startIndex, startIndex + ITEMS_PER_PAGE);
    ...
    if (sales.length === 0) {
        return (
            <div className="bg-card-bg rounded-xl border border-border-light p-10 text-center shadow-sm">
                ...
                <h3 className="font-semibold text-text-primary">No sales found</h3>
                <p className="text-sm text-text-muted mt-1">Record your first sale to get started</p>
            </div>
        );
    }
                    ...
                    <tbody className="divide-y divide-border-light text-sm">
                        {paginated.map((s) => (
                            ...
                            >
                                ...
                                <td className="px-4 py-1 font-medium group-hover:text-[#f67315] transition-colors text-text-primary">
                                    {s.customer_name || 'Walk-in Customer'}
                                </td>
                                <td className="px-4 py-1 text-center hidden sm:table-cell text-text-secondary">
                                    {s.item_count || 0}
                                </td>
                                <td className="px-4 py-1 text-right font-bold text-text-primary">
                                    {formatCurrency(s.total_amount)}
                                </td>
                                <td className="px-4 py-1 text-right hidden md:table-cell text-emerald-600">
                                    {formatCurrency(s.paid_amount)}
                                </td>
                                <td className="px-4 py-1 text-right hidden md:table-cell font-medium text-red-600">
                                    {formatCurrency(s.remaining_amount)}
                                </td>
                                <td className="px-4 py-1 text-center hidden lg:table-cell">
                                    <StatusBadge paid={s.paid_amount} remaining={s.remaining_amount} />
                                </td>
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-between px-4 py-3 border-t border-border-light bg-app-surface-alt/30 rounded-b-xl">
                    <span className="text-[11px] text-text-muted font-medium">
                        Showing <span className="font-bold text-text-primary">{startIndex + 1}</span>–<span className="font-bold text-text-primary">{Math.min(startIndex + ITEMS_PER_PAGE, sales.length)}</span> of <span className="font-bold text-text-primary">{sales.length}</span>
                    </span>
