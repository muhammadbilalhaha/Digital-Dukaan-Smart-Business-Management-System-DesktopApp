// src/features/customers/components/CustomerPaymentModal.jsx
...
const CustomerPaymentModal = ({ customer, isOpen, onClose, onSubmit, isSubmitting }) => {
    const { 
        register, 
        handleSubmit, 
        formState: { errors }, 
    ...
    const remainingDue = customer.total_due || 0;
                    ...
                    <div className="flex items-center gap-3.5">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-text-muted mt-0.5">
                                Customer: <span className="font-medium text-text-secondary">{customer.name}</span>
                            </p>
                        </div>
                    </div>
                    <button 
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-5">
                    ...
                    <div className="bg-app-surface-alt rounded-2xl p-4 border border-border-light space-y-3">
                        ...
                        <div className="grid grid-cols-3 gap-2 pt-2 border-t border-border-light/60">
                            <div>
                                <span className="text-[10px] uppercase font-bold text-text-muted block">Total Purchase</span>
                                <span className="text-xs font-semibold text-text-primary">{formatCurrency(customer.total_purchase)}</span>
                            </div>
                            <div>
                                <span className="text-[10px] uppercase font-bold text-text-muted block">Total Paid</span>
                                <span className="text-xs font-semibold text-emerald-600">
                                    {formatCurrency((customer.total_purchase || 0) - (customer.total_due || 0))}
                                </span>
                            </div>
                            <div>
                                <span className="text-[10px] uppercase font-bold text-text-muted block">Remaining Due</span>
                                <span className="text-xs font-bold text-red-600">{formatCurrency(customer.total_due)}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="relative rounded-xl shadow-sm">
                            <div className="absolute inset-y-0 left-0 pl-3.5 font-bold mr-5 flex items-center pointer-events-none text-text-muted">
                                RS.
                            </div>
                            <input 
                                type="number" 
                                ...
                                placeholder="0.00" 
                                className={`w-full bg-input-bg border text-text-primary placeholder:text-text-muted text-sm font-semibold rounded-xl pl-10 pr-4 py-3 focus:outline-none focus:ring-2 transition-all ${
                                    errors.amount 
